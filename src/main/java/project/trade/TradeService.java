...
public class TradeService {
    ...
    // 안전 결제 상태 조회 (FOR UPDATE 쿼리이므로 readOnly 해제)
    @Transactional
    ...
    /** 모바일 전용: 임의 SaleStatus로 상태 변경 (SALE / RESERVED / SOLD) */
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = "trade", key = "#trade_seq"),
            @CacheEvict(value = "tradeList", allEntries = true),
    ...
    public boolean updateSaleStatus(long trade_seq, SaleStatus newStatus) {
        return tradeMapper.statusUpdate(trade_seq, newStatus) > 0;
    }
    ...
    public void completePurchaseAndNotify(Long tradeSeq, long buyerSeq, int amount, String postNo, String addrH, String addrD) {
        ...
        int updated = tradeMapper.successPurchase(tradeSeq, buyerSeq, postNo, addrH, addrD);
        if (updated == 0) {
            ...
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                ...
                public void afterCommit() {
                    try {
                        ...
                        log.error("Failed to delete image after commit: {}, error: {}", url, e.getMessage());
                    }
                }
            });
        } else {
