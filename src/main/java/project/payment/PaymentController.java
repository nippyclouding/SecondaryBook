...
public class PaymentController {
    ...
    public String success(@RequestParam Long trade_seq,
        ...
        if (buyer == null) {
            redirectAttributes.addFlashAttribute("errorMessage", "세션 누락");
            return "redirect:/payments/result?status=fail";
        }

        // 검증 - 안전결제 대상 구매자인지 재확인 (IDOR 방지)
        PaymentVO paymentCheck = tradeService.getPaymentCheckInfo(trade_seq);
        if (paymentCheck == null || paymentCheck.getPendingBuyerSeq() == null
                || !paymentCheck.getPendingBuyerSeq().equals(buyer.getMember_seq())) {
        ...
        if ("manual".equals(addr_type) || "existing".equals(addr_type)) {
            if (addr_h == null || addr_d == null || post_no == null
                ...
                redirectAttributes.addFlashAttribute("errorMessage", "배송지 정보가 올바르지 않습니다.");
                return "redirect:/payments/result?status=fail";
            }
            ...
            redirectAttributes.addFlashAttribute("errorMessage", "배송지 타입이 올바르지 않습니다.");
            return "redirect:/payments/result?status=fail";
        }
    ...
    private void sendPaymentFailedMessage(Long trade_seq, Long member_seq) {
        try {
            ...
            if (chat_room_seq != null) {
                ...
                chatMessagePublisher.publishPayment(chat_room_seq, failMsg); // Redis Pub/Sub
