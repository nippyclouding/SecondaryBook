...
[3단계 - Spring Batch, 매일 새벽 3시 자동 실행]
  ...
  └─ 판매자 정산 신청
       └─ REQUESTED  ─────────────────────────────────────────────────────────────────────────┐
            └─ 새벽 3시 배치                                                                      │
                 ...
                 └─ 잔액 부족  → INSUFFICIENT_BALANCE → 관리자 잔액 충전 후 재처리 설정 ────────────────┘
```
  ...
  └─ SettlementScheduler
       └─ JobLauncher.run(settlementJob)
            └─ processSettlementStep (chunk = 1, 1건 = 1 트랜잭션)
                 │
                 ├─ [Reader]      REQUESTED 상태 정산 건 전체를 DB에서 한 번에 조회
                 │                 → 이후 1건씩 순차적으로 Processor에 전달
                 │
...
| 서버 2대 이상(Auto Scaling) 환경 | 두 서버 모두 새벽 3시에 실행되면 이중 처리 | `@SchedulerLock` (ShedLock)으로 1대만 실행 보장 |
...
### Spring Batch — @Scheduled와의 역할 분리, chunk=1 선택 이유

> 상세 플로우는 [정산 플로우 섹션](#정산-플로우)을 참고하세요.

**@Scheduled vs Spring Batch 역할 분리**

`@Scheduled(cron = "0 0 3 * * *")`는 배치를 새벽 3시에 실행시키는 **트리거**일 뿐이다.
실제 처리(트랜잭션 관리, 실패 skip, 이중 처리 방지, 이력 기록)는 **Spring Batch**가 담당한다.
`@Scheduled` 안에서 반복문으로 직접 처리하면 실패 시 전체 롤백, 이중 처리 위험, 이력 관리 부재 등의 문제가 생긴다.
