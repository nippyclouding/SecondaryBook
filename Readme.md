...
- **수동 이체 + 확인** - 관리자가 은행 앱으로 직접 이체 후 "정산 완료 처리" 클릭

### 8. 관리자
- **대시보드** - 회원 수, 거래 수, 모임 수 통계
- **회원/상품/모임 관리** - 목록, 검색, 상세 조회, 회원 제재
- **안전결제 내역** - 전체 결제 내역 조회
- **정산 관리** - 이체 대기 목록, 정산 완료 내역, 계좌번호 복호화 표시
- **공지사항/배너 관리** - CRUD
...
[2단계 - 판매자]
  ...
  └─ 검증 통과 시
      ...
      └─ trade.settlement_st → REQUESTED (관리자 이체 대기)

[3단계 - 관리자]
  관리자 페이지 > 정산 관리 > 이체 대기 목록 확인
  └─ 계좌번호(복호화), 예금주, 은행 정보 확인
  └─ 은행 앱에서 직접 이체
  └─ "정산 완료 처리" 버튼 클릭 → settlement_st = COMPLETED → 처리 종료
```
...
<img width="841" height="820" alt="SystemArchitecture" src="https://github.com/user-attachments/assets/2adf055f-f46b-4878-9a23-2047747e5618" />
