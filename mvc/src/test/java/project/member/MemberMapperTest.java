...
class MemberMapperTest {
    ...
    class SignUp {
        ...
        void signUp_returns1() {
            ...
            int result = memberMapper.save(vo);
    ...
    // ========================================================================
    @Nested
    @DisplayName("중복 체크 (countByLoginId / countByEmail / countByNickname)")
    class DuplicateCheck {
        ...
        @Test
        @DisplayName("countByLoginId - 존재하는 ID → 1")
        void idCheck_exists() {
            assertThat(memberMapper.countByLoginId("checkUser")).isEqualTo(1);
        }

        @Test
        @DisplayName("countByLoginId - 없는 ID → 0")
        void idCheck_notExists() {
            assertThat(memberMapper.countByLoginId("unknown")).isEqualTo(0);
        }

        @Test
        @DisplayName("countByEmail - 존재하는 이메일 → 1")
        void emailCheck_exists() {
            assertThat(memberMapper.countByEmail("check@test.com")).isEqualTo(1);
        }

        @Test
        @DisplayName("countByEmail - 없는 이메일 → 0")
        void emailCheck_notExists() {
            assertThat(memberMapper.countByEmail("none@test.com")).isEqualTo(0);
        }

        @Test
        @DisplayName("countByNickname - 존재하는 닉네임 → 1")
        void nickNmCheck_exists() {
            assertThat(memberMapper.countByNickname("체크닉")).isEqualTo(1);
        }

        @Test
        @DisplayName("countByNickname - 없는 닉네임 → 0")
        void nickNmCheck_notExists() {
            assertThat(memberMapper.countByNickname("없는닉")).isEqualTo(0);
        }
    }
    ...
    // ========================================================================
    @Nested
    @DisplayName("countByLoginIdAndEmail - ID + 이메일 매칭")
    class CheckUserByIdAndEmail {
        ...
        void match_returns1() {
            ...
            int result = memberMapper.countByLoginIdAndEmail("matchUser", "match@test.com");
            assertThat(result).isEqualTo(1);
        }
        ...
        void emailMismatch_returns0() {
            ...
            int result = memberMapper.countByLoginIdAndEmail("mismatch", "wrong@test.com");
            assertThat(result).isEqualTo(0);
        }
    }

    // ========================================================================
    // updateLastLogin - 마지막 로그인 시간 갱신
    // ========================================================================
    @Test
    @DisplayName("updateLastLogin - 반환값 1 (last_login_dtm 갱신)")
    void updateLastLogin_returns1() {
        insertMember("logUser", "log@test.com", "로그회원");
        long seq = memberMapper.findByLoginId("logUser").getMember_seq();

        int result = memberMapper.updateLastLogin(seq);
