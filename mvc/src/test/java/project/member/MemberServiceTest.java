...
class MemberServiceTest {
    ...
    class SignUp {
        ...
        void 비밀번호_암호화_저장() {
            ...
            when(memberMapper.save(vo)).thenReturn(1);
        ...
        void 가입실패_false() {
            ...
            when(memberMapper.save(vo)).thenReturn(0);
    ...
    class Login {
        ...
        void MD5회원_로그인_마이그레이션() {
            ...
            when(memberMapper.findByMd5Password(input)).thenReturn(dbMember);
        ...
        void MD5회원_비밀번호불일치() {
            ...
            when(memberMapper.findByLoginId("oldUser")).thenReturn(dbMember);
            when(memberMapper.findByMd5Password(input)).thenReturn(null);
    ...
    class DuplicateCheck {
        ...
        void idCheck_중복() {
            when(memberMapper.countByLoginId("existUser")).thenReturn(1);
            assertThat(memberService.countByLoginId("existUser")).isEqualTo(1);
        }

        @Test
        @DisplayName("ID 사용 가능 - 0 반환")
        void idCheck_사용가능() {
            when(memberMapper.countByLoginId("newUser")).thenReturn(0);
            assertThat(memberService.countByLoginId("newUser")).isEqualTo(0);
        }

        @Test
        @DisplayName("닉네임 중복 - 1 반환")
        void nickNmCheck_중복() {
            when(memberMapper.countByNickname("existNick")).thenReturn(1);
            assertThat(memberService.countByNickname("existNick")).isEqualTo(1);
        }

        @Test
        @DisplayName("이메일 중복 - 1 반환")
        void emailCheck_중복() {
            when(memberMapper.countByEmail("exist@test.com")).thenReturn(1);
            assertThat(memberService.countByEmail("exist@test.com")).isEqualTo(1);
        }
    }
    ...
    class CheckUserByIdAndEmail {
        ...
        void 일치_true() {
            when(memberMapper.countByLoginIdAndEmail("testUser", "test@test.com")).thenReturn(1);
        ...
        void 불일치_false() {
            when(memberMapper.countByLoginIdAndEmail("noUser", "no@test.com")).thenReturn(0);
    ...
    class ProcessSocialLogin {
        ...
        void 기존회원_바로로그인() {
            ...
            when(memberMapper.findByOAuth(params)).thenReturn(existMember);
            ...
            verify(memberMapper, never()).saveSocialMember(any());
        }
        ...
        void 탈퇴한회원_null() {
            ...
            when(memberMapper.findByOAuth(params)).thenReturn(deletedMember);
        ...
        void 신규회원_가입후로그인() {
            ...
            when(memberMapper.findByOAuth(params))
                    .thenReturn(null)
        ...
        void 닉네임중복_난수추가() {
            ...
            when(memberMapper.findByOAuth(params)).thenReturn(null).thenReturn(new MemberVO());
            when(memberMapper.countByNickname("duplicateNick")).thenReturn(1);
            when(memberMapper.countByNickname(argThat(nick -> nick.startsWith("duplicateNick_")))).thenReturn(0);
